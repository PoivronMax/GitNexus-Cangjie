...
export const findEnclosingClassId = (node, filePath) => {
    ...
    while (current) {
        ...
        if (current.type === 'classDefinition') {
            ...
            if (cn?.text)
                return generateId('Class', `${filePath}:${cn.text}`);
        }
        if (current.type === 'interfaceDefinition') {
            const n = current.namedChildren?.find((c) => c.type === 'interfaceName');
            if (n?.text)
                return generateId('Interface', `${filePath}:${n.text}`);
        }
        if (current.type === 'structDefinition') {
            const n = current.namedChildren?.find((c) => c.type === 'structName');
            if (n?.text)
                return generateId('Struct', `${filePath}:${n.text}`);
        }
        if (current.type === 'enumDefinition') {
            const n = current.namedChildren?.find((c) => c.type === 'enumName');
            if (n?.text)
                return generateId('Enum', `${filePath}:${n.text}`);
        }
        if (current.type === 'extendDefinition') {
            const et = current.namedChildren?.find((c) => c.type === 'extendType');
            if (et) {
                const idNode = et.namedChildren?.find((c) => c.type === 'identifier' || c.type === 'scoped_identifier');
                if (idNode?.text)
                    return generateId('Class', `${filePath}:${idNode.text}`);
            }
        }
...
export const getLanguageFromFilename = (filename) => {
    ...
    if (filename.endsWith('.cj'))
        return SupportedLanguages.Cangjie;
    // C source files
